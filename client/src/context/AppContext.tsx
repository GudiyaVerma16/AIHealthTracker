...
export const AppProvider = ({children} : {children: React.ReactNode})=>{
    ...
    const login = async (credentials: Credentials)=>{
        try {
            const { data } = await api.post('/auth/local', {identifier: credentials.email, password: credentials.password})
    ...
    const fetchFoodLogs = async (token: string)=>{
        try {
            const {data} = await api.get('/food-logs', {headers: { Authorization: `Bearer ${token}` }})
            setAllFoodLogs(data)
    ...
    const fetchActivityLogs = async (token: string)=>{
        try {
            const {data} = await api.get('/activity-logs', {headers: { Authorization: `Bearer ${token}` }})
